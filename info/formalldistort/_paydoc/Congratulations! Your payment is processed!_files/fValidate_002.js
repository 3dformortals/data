...
------------------------------------------- */
//chosenElements - pass false to validate whole form, pass an array of elements to validate them
function validateForm( f, bConfirm, bDisable, bDisableR, groupError, errorMode, chosenElements )
{
	...
	errorMode	= ( typeof errorMode != 'undefined' ) ? parseInt( errorMode, 10 ) : 8;
	...
	var elem, i = 0;
	var elemSrc = chosenElements ? chosenElements : dataBinder.ValidateFields;
	for (eid in elemSrc)  
	{
		var elem = document.getElementById(eid);
		if (!elem || !dataBinder.ValidateFields[eid] || isElementHidden(elem)) continue;
		//	Does element have validator attribute? (short-circuit check)
		fvCode			= dataBinder.ValidateFields[eid].vid;
		if ( !( typeof fvCode == 'undefined' || fvCode == null || fvCode == "" ) )
		{
			...
			{
				var temparams = type.split( "validate" );
				type = temparams.last();
			...
			elem.fName	= dataBinder.ValidateFields[eid].title;
			f.fv.elem		= elem;
			f.fv.type		= type;
			...
			//	Check for blank too
			if ( !elem.bok && f.fv.type !="blank" ) {
			    stype = 'blank';
				f.fv.type	= stype;

				//	Create function to call the proper validator method of the fValidate class
				var func = new Function( "obj", "method", "obj[method]();" );
				func( f.fv, stype );
			}
...
/*			if ( typeof fvCode != 'undefined' && fvCode == "" && !elem.bok ) {
			    type = 'blank';
				//	Set formatted name, current element
...
fValidate.prototype.throwError = function( args, which )
{
	...
	if(this.checkErrors(elem))
	{
		...
		var error = (which == -1 )
			? this.translateMessage( args, args[0] )
			: this.translateMessage( args, this.i18n.errors[this.type][which] );
...
/*Check stack of errors for particular element
true - no errors for this element
false - error found for this element
--------------------------------------------*/
fValidate.prototype.checkErrors = function (elem)
{
	for(var i=0; i<this.errors.length; i++){
		if(this.errors[i]['elem'].id == elem.id) return false;
	}
	return true;
}
...
fValidate.prototype.showError = function( emsg, last, elem )
{
	...
	function inputError()
	{
		...
		{
			//Find if there is "Empty Value" fake element
			if ( singleCSS )
			{
		...
		var emsg_id = 'errorMsg_'+elem.id;
		if(!document.getElementById(emsg_id)){
			var tr = getParentTR(elem);
			var emsgHTML = '<div id="'+emsg_id+'">'+ emsg + '</div>';
			var error_placeholder = document.getElementById('errorPH_' + tr.id);
			if (error_placeholder){
				error_placeholder.innerHTML = error_placeholder.innerHTML + emsgHTML;
				error_placeholder.parentNode.parentNode.style.display='';
			}
		}
	}
	...
	function boxError()
	{
		...
		if (self.boxError.parentNode.style.display!='block'){
			var captionEl = self.boxError.getElementsByTagName('SPAN')[0];
			captionEl.innerHTML = self.i18n.boxCaption;
			captionEl.parentNode.innerHTML = captionEl.parentNode.innerHTML + '<br>' + self.i18n.boxMessageText;
			self.boxError.parentNode.style.display='block';
			window.location.hash = 'top';
		}
		//Unfold all multi-valued fields
		for (var i in dataBinder.ModeSwitchers){
			switchModeAll(i);
		}
...
{
	if ( document.getElementById('input___' + elem.name))
		document.getElementById('input___' + elem.name).className = elem.className.replace( className, "" ).trim();
	if ( document.getElementById('td___' + elem.name))
		document.getElementById('td___' + elem.name).className = elem.className.replace( className, "" ).trim();
...
fValidate.prototype.addCSSClass = function( elem, className )
{
	...
	if ( document.getElementById('input___' + elem.name))
		document.getElementById('input___' + elem.name).className = ( elem.className + " " + className ).trim();
	if ( document.getElementById('td___' + elem.name))
		document.getElementById('td___' + elem.name).className = ( elem.className + " " + className ).trim();
...
fValidate.prototype.revertError = function( elem )
{
	...
	errorPH = document.getElementById('errorMsg_'+elem.id);	
	if(errorPH){
		errorPH.parentNode.removeChild(errorPH);
	}
	...
	if ( ( typeof elem.length != 'undefined' && elem.length > 1 && elem.nodeName != 'SELECT' ) || isHidden )
	{
		...
		{
			if ( typeof elem.revertClass != 'undefined' )
			{
...
{
	var pats = {
		'text':		'text|password|textarea|hidden|file|select',
		'ta':		'textarea',
		'hidden':	'hidden',
		's1':		'select-one',
		'sm':		'select-multiple',
...
------------------------------------------- */
fValidate.prototype.translateMessage = function( args, rawtext )
{
	stmp	= new String(rawtext);
	pattern = /\$\d+/;
	var start = stmp.match(/\$0/) ? 0 : 1;
	var tokens = stmp.match(/\$\d+/g);
	if ( tokens )
		for ( var i = 0; i < tokens.length; i++ )
		{  	
    		var item = parseInt(tokens[i].replace( /\$(\d+)/, "$1"));
			stmp = stmp.replace(tokens[i], args[start + item]);
		}
	return stmp;
}
...
/***************************************************************************/
// Parallels Special function used for bok
...
****************************************************************************/
function addError(o,descerrorstr) {
	errorData[errorData.length] = o;
	var temp_name = formatName(o);  
	descerror[temp_name] = descerrorstr;
}
/***************************************************************************/
