...
var d ; //диаметр сечения звена сегмента кубита
...
VZ=Math.PI*Math.pow(d,2)*(a-d)/4;
SZ=Math.PI*d*(a-d);
SU=Math.PI*Math.pow((d*Math.sqrt(2)),2)-2*Math.PI*d/Math.sqrt(2)*(d/Math.sqrt(2)-d/2)*6;
MZ=ro*VZ;
MU=ro*VU;
VZK=VZ*nZ;
MZK=MZ*nZ;
...
SUG=UG*Math.PI*Math.pow((d/2),2);
UR=(nx-1)*4+(ny-1)*4+(nz-1)*4;
SUR=UR*Math.PI*Math.pow((d/2),2)*2;
UV=8;
SUV=UV*Math.PI*Math.pow((d/2),2)*3;

SPov=SZK+SUK+SUG+SUR+SUV;
SPG=(a-d)*(a-d);
SPro=SPG*nG;
Lpro=a-d;
