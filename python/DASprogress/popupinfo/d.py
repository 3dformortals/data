...
class ItemScanner(object):
    def __init__(self, parent, prompt, tabname, ind):
        ...
        webname,website,canscan,scanstatus,webready,webnew = web_scanner(self.idone,self.ihref)
        text = ""
        text+="name: "+self.iname.get()
        text+="\nwebname: "+webname
        text+="\nwebsite: "+website
        text+="\ncan scan: "+canscan
    ...
    def onclick_web(self, event):
        try:
            webbrowser.open(self.ihref.get())
        except:
            print("error onclick_web\n", sys.exc_info())
...
img = tkinter.Image("photo", file=mydir+os.sep+"icon.png")
mainframe.tk.call('wm','iconphoto',mainframe._w,img)
